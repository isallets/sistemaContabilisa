    ...
    const contasPadrao: Conta[] = [
      {
        ...
        grupo_contabil: "Ativo",
        subgrupo1: "Ativo Circulante",
        subgrupo2: "Disponibilidades",
      },
      {
        id: 2,
        codigo: "2.1.01.001",
        nome_conta: "Fornecedores Nacionais",
        grupo_contabil: "Passivo",
        subgrupo1: "Passivo Circulante",
        subgrupo2: "Obrigações",
      },
      {
        id: 3,
        codigo: "3.1.01.001",
        nome_conta: "Capital Social",
        grupo_contabil: "Patrimônio Líquido",
        subgrupo1: "Capital",
        subgrupo2: "-",
      },
    ];
...
// --- ROTAS CONTAS ---
app.get("/api/contas", async (_req: Request, res: Response) => {
  const snapshot = await db.ref("contas").once("value");
...
// --- BALANCETE ---
app.get("/api/balancete", async (_req: Request, res: Response) => {
  const contasSnap = await db.ref("contas").once("value");
  const lancSnap = await db.ref("lancamentos").once("value");
  ...
  const balancete = contas.map((conta) => {
    const totalDebito = lancamentos
      .filter((l) => l.contaDebitoId === conta.id)
      .reduce((s, l) => s + l.valor, 0);
    const totalCredito = lancamentos
      .filter((l) => l.contaCreditoId === conta.id)
      .reduce((s, l) => s + l.valor, 0);
    return {
      codigo: conta.codigo,
...
// --- BALANÇO PATRIMONIAL ---
app.get("/api/balanco-patrimonial", async (_req: Request, res: Response) => {
  try {
    const contasSnap = await db.ref("contas").once("value");
    const lancSnap = await db.ref("lancamentos").once("value");
    ...
    const firebaseObjectToArray = (snapshotVal: object) => {
      ...
      return Object.entries(snapshotVal).map(([id, data]) => ({
        id,
        ...data,
      }));
    };
    ...
    const saldos = contas
      .map((conta: any) => {
        const totalDebito = lancamentos
          .filter((l: any) => l.contaDebitoId === conta.id)
          .reduce((s, l: any) => s + l.valor, 0);
        const totalCredito = lancamentos
          .filter((l: any) => l.contaCreditoId === conta.id)
          .reduce((s, l: any) => s + l.valor, 0);
    ...
    const relatorioFinal = {
      ativo: {} as Record<string, any>,
      passivo: {} as Record<string, any>,
      patrimonioLiquido: {} as Record<string, any>,
    };

    const ativoPorSubgrupo1 = groupBy(
      saldos.filter((c: any) => c.grupo_contabil === "Ativo"),
      "subgrupo1"
    );
    for (const subgrupo1 in ativoPorSubgrupo1) {
      relatorioFinal.ativo[subgrupo1] = groupBy(
        ativoPorSubgrupo1[subgrupo1],
        "subgrupo2"
      );
    }

    const passivoPorSubgrupo1 = groupBy(
      saldos.filter((c: any) => c.grupo_contabil === "Passivo"),
      "subgrupo1"
    );
    for (const subgrupo1 in passivoPorSubgrupo1) {
      relatorioFinal.passivo[subgrupo1] = groupBy(
        passivoPorSubgrupo1[subgrupo1],
        "subgrupo2"
      );
    }

    const plPorSubgrupo1 = groupBy(
      saldos.filter((c: any) => c.grupo_contabil === "Patrimônio Líquido"),
      "subgrupo1"
    );
    for (const subgrupo1 in plPorSubgrupo1) {
      relatorioFinal.patrimonioLiquido[subgrupo1] = groupBy(
        plPorSubgrupo1[subgrupo1],
        "subgrupo2"
      );
    }
    ...
    return res
      .status(500)
      .json({ message: "Erro interno no servidor ao gerar balanço." });
  }
});
...
app.get("/api/livro-razao/:contaId", async (req: Request, res: Response) => {
  ...
  const movimentos = lancamentos
    ...
    .map((l) => ({
      ...
      debito: l.contaDebitoId === contaId ? l.valor : 0,
      credito: l.contaCreditoId === contaId ? l.valor : 0,
    }));
